...
public class Emerald1 {
    ...
    private static void notifyPlayers(Game game) {
        game.getPlayers().stream()
                ...
                .forEach(player -> {
                    String message = gametask_mess_emerald1_chat;
                    if (message != null && !message.isEmpty()) {
                        sendMessage(player,message);
                    }

                    String title = Optional.ofNullable(gametask_mess_emerald1_title).orElse(" ");
                    String subtitle = Optional.ofNullable(gametask_mess_emerald1_subtitle).orElse("");

                    sendTitle(player, title, subtitle);

                    Optional.ofNullable(gametask_mess_emerald1_actionbar)
