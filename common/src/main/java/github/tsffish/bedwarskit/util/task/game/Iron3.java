...
public class Iron3 {
    ...
    private static void notifyPlayers(Game game) {
        game.getPlayers().stream()
                ...
                .forEach(player -> {
                    String message = gametask_mess_iron3_chat;
                    if (message != null && !message.isEmpty()) {
                        sendMessage(player,message);
                    }

                    String title = Optional.ofNullable(gametask_mess_iron3_title).orElse(" ");
                    String subtitle = Optional.ofNullable(gametask_mess_iron3_subtitle).orElse("");

                    sendTitle(player, title, subtitle);

                    Optional.ofNullable(gametask_mess_iron3_actionbar)
