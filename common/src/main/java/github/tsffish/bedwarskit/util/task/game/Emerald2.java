...
public class Emerald2 {
    ...
    private static void notifyPlayers(Game game) {
        game.getPlayers().stream()
                ...
                .forEach(player -> {
                    String message = gametask_mess_emerald2_chat;
                    if (message != null && !message.isEmpty()) {
                        player.sendMessage(message);
                    }

                    String title = Optional.ofNullable(gametask_mess_emerald2_title).orElse(" ");
                    String subtitle = Optional.ofNullable(gametask_mess_emerald2_subtitle).orElse("");

                    player.sendTitle(title, subtitle);

                    Optional.ofNullable(gametask_mess_emerald2_actionbar)
