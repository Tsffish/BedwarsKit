...
public class Gold2 {
    ...
    private static void notifyPlayers(Game game) {
        game.getPlayers().stream()
                ...
                .forEach(player -> {
                    String message = gametask_mess_gold2_chat;
                    if (message != null && !message.isEmpty()) {
                        sendMessage(player,message);
                    }

                    String title = Optional.ofNullable(gametask_mess_gold2_title).orElse(" ");
                    String subtitle = Optional.ofNullable(gametask_mess_gold2_subtitle).orElse("");

                    sendTitle(player, title, subtitle);

                    Optional.ofNullable(gametask_mess_gold2_actionbar)
