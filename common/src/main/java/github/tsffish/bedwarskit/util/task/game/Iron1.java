...
public class Iron1 {
    ...
    private static void notifyPlayers(Game game) {
        game.getPlayers().stream()
                ...
                .forEach(player -> {
                    String message = gametask_mess_iron1_chat;
                    if (message != null && !message.isEmpty()) {
                        player.sendMessage(message);
                    }

                    String title = Optional.ofNullable(gametask_mess_iron1_title).orElse(" ");
                    String subtitle = Optional.ofNullable(gametask_mess_iron1_subtitle).orElse("");

                    player.sendTitle(title, subtitle);

                    Optional.ofNullable(gametask_mess_iron1_actionbar)
